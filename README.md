...
这是一个 Next.js 全栈文案生成工具。浏览器只调用本项目后端接口，后端再调用 OpenClaw Gateway。
...
`OPENCLAW_API_TOKEN` 只在服务端读取，不会暴露给前端。
...
后端在 `app/api/generate/route.ts` 中组装 prompt，然后通过 `lib/openclaw.ts` 调用 OpenClaw。
